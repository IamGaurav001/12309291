...
const sampleVehicles = [
    ...
    { TaskID: "f92b0f39-35ec-47c3-a465-3e49c2285b6", Duration: 2, Impact: 5 },
...
async function getVehicles() {
    let url = `${config.baseUrl}/vehicles`;
    logger.info('service', `Fetching vehicles from: ${url}`);
    ...
    try {
        ...
        logger.info('service', `Got ${res.data?.vehicles?.length || 0} vehicles from API`);
        return res.data.vehicles || sampleVehicles;
    } catch (err) {
        logger.warn('service', `Vehicle API failed (${err.message}), using local data`);
